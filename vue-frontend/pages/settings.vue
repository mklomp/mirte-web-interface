                ...
                <tbody>
                  <template v-for="(set_peripherals, type) in items" :key="type">
                    <tr v-for="(item, name) in set_peripherals" :key="name">
                      ...
                      <td>
                        <div v-for="p in Object.keys(peripherals[type].pins)" :key="p" class="mb-2">
                          <select class="form-select" v-model="item.pins[p]">
                            <option :value="null" disabled>{{ p }}</option>
                            <option v-for="opt in getValidPinBinds(type, p)" :key="opt.value" :value="opt.value">
                              {{ opt.text }}
                            </option>
                          </select>
                        </div>
...
export default {
  ...
  methods: {
    ...
    uploadMCU() {
      ...
      .then(data => {
        this.busy = false
        alert(data.toLowerCase().includes("download done") ? "Uploaden is succesvol afgerond" : "Er is een fout opgetreden:\n\n" + data)
      })
    },
