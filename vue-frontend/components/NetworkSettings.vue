					...
					<div class="row before-submit">
						<div class="col">
							<form class="form form-horizontal" id='connect-form' :action="`http://${this.ip}/wifi-connect/connect`"
								...
								<div class="form-group">
									...
									<div class="col">
										<select id='ssid-select' class="form-control" name='ssid' @change="onChange($event)"
											...
											<option v-for="n of networks" v-bind:key="n.ssid" :value="n.ssid" :data-security="n.security">
												{{ n.ssid }}</option>
										</select>
									</div>
								</div>
								...
								<div class="form-group">
									<div class="col">
										<button @click="connect" type="button" class="btn btn-mirte">{{ $t('settings.wifi_connect')
										}}</button>
									</div>
								</div>
							</form>
